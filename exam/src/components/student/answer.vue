...
export default {
  data() {
    return {
      ...
      endTime: null, // 考试结束时间
      time: null, // 考试持续时间
      reduceAnswer: [], // vue官方不支持3层以上数据嵌套,此变量直接接收3层嵌套时的数据。
      answerScore: 0, // 答题总分数
      ...
      rightAnswer: '',
      hiddenTimer: null, // 定时器ID
      popStateTimer: null, // 监听popstate事件的定时器ID
      prevPath: '' // 上一次的路径
    };
  },
  created() {
    ...
    this.getExamData();
    this.showTime();
    this.prevPath = window.location.href;
  ...
  methods: {
    handleVisibilityChange() {
      if (document.hidden) {
        ...
        this.hiddenTimer = setTimeout(() => {
          console.log("离开考试已超时,强制交卷。");
          this.autoCommit(); // 自动提交考试
        }, 2000);
      } else {
        // 页面重新可见时清除定时器
    ...
    handlePopState() {
      ...
      if (currentPath !== this.prevPath) {
        this.popStateTimer = setTimeout(() => {
          console.log("浏览器返回超时,强制交卷。");
          this.autoCommit(); // 自动提交考试
        }, 2000);
      }
    },
    ...
    getRadioLabel(val) { // 获取选择题作答选项
      let data = this.topic[1];
      this.bg_flag = true;
      if (data[this.index]["isClick"] != true) {
    ...
    calculateScore() { // 计算得分
      ...
      this.answerScore = finalScore; // 保存总分数
    },
    submitScore(showMessage) { // 提交成绩信息
